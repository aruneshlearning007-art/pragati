  ...
  async function handleLogin() {
    ...
    try {
      const res = await fetch("/api/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: loginEmail, role: "student" }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(data?.error || "Something went wrong. Please try again.");
      ...
      <div className="max-w-md w-full bg-surface border border-border rounded-card p-10 shadow-sm">
        ...
        {mode === "signup" ? (
          <>
            ...
            <Field label={t.email} hint={t.emailHintStudent}>
              <input
                type="email"
                className="input"
                value={form.email}
          ...
          <>
            <h1 className="font-heading text-2xl font-semibold mb-1">{t.loginTitleStudent}</h1>
